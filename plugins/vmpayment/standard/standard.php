...
class plgVmPaymentStandard extends vmPSPlugin
{
    ...
    function plgVmOnShowOrderBEPayment($virtuemart_order_id, $virtuemart_payment_id)
    {
        ...
        $db->setQuery("SELECT exchange_usd FROM #__virtuemart_orders WHERE virtuemart_order_id = $virtuemart_order_id");
        $exchangeUsd = $db->loadResult();
        ...
        $price = round($exchangeUsd * $paymentTable->payment_order_total, 2).' KGS';
        $price .= ' / ';
        $price .= round($paymentTable->payment_order_total, 2).' '.$paymentTable->payment_currency;

        $html = '<table class="adminlist table">'."\n";
        $html .= $this->getHtmlHeaderBE();
        $html .= $this->getHtmlRowBE('COM_VIRTUEMART_PAYMENT_NAME', $paymentTable->payment_name);
        $html .= $this->getHtmlRowBE('STANDARD_PAYMENT_TOTAL_CURRENCY', $price);
        $html .= '</table>'."\n";
