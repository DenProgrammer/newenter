...
class modDJImageSliderHelper
{
    ...
    static function getSlideDescription($slide, $limit)
    {
        ...
        if ($sparams->get('link_type', '') == 'article' && empty($slide->description)) { // if article and no description then get introtext as description
            if (isset($slide->introtext))
                $slide->description = $slide->introtext;
        }
    ...
    static function getNavigation(&$params, &$mid)
    {
        ...
        if ($params->get('slider_type') == 1) {
            if (empty($prev) || !file_exists(JPATH_ROOT.DS.$prev))
                $prev = JURI::base().'/modules/mod_djimageslider/assets/up.png';
            if (empty($next) || !file_exists(JPATH_ROOT.DS.$next))
                $next = JURI::base().'/modules/mod_djimageslider/assets/down.png';
        } else {
            if (empty($prev) || !file_exists(JPATH_ROOT.DS.$prev))
                $prev = JURI::base().'/modules/mod_djimageslider/assets/prev.png';
            if (empty($next) || !file_exists(JPATH_ROOT.DS.$next))
                $next = JURI::base().'/modules/mod_djimageslider/assets/next.png';
        }
        if (empty($play) || !file_exists(JPATH_ROOT.DS.$play))
            $play  = JURI::base().'/modules/mod_djimageslider/assets/play.png';
        if (empty($pause) || !file_exists(JPATH_ROOT.DS.$pause))
            $pause = JURI::base().'/modules/mod_djimageslider/assets/pause.png';
    ...
    static function getStyleSheet(&$params, &$mid)
    {
		...
		';
        if ($params->get('slider_source') && ($params->get('show_title') || ($params->get('show_desc'))))
            $css.= '
		...
		';
        if ($params->get('show_buttons') || $params->get('show_arrows'))
            $css .= '
		/* Navigation buttons */
		#navigation'.$mid.' {
			position: relative;
