...
class simpleForm2 extends JObject {
        ...
        foreach ($matches[1] as $key => $paramsText) {
            ...
            foreach ($matchesP[1] as $keyP => $paramName) {
                if (in_array($paramName, $paramNames)) {
                    ...
                    if (in_array($paramName, $params2mask))
                        $elem->$paramName = base64_decode($elem->$paramName);
                    if ($paramName == 'label') {
                    ...
                    if (is_array($matchesOP[1]) && count($matchesOP[1]) > 0) {
                        ...
                        foreach ($matchesOP[1] as $keyP => $paramName) {
                            if (in_array($paramName, $optionParamNames)) {
                                ...
                                if (in_array($paramName, $params2mask))
                                    $option->$paramName = base64_decode($option->$paramName);
            ...
            if ($elem->type == 'captcha') {
                if (!isset($elem->color) || !preg_match("/\#?[0-9ABCDEFabcdef]{6}/", $elem->color))
                    $elem->color      = '';
                if (!isset($elem->background) || !preg_match("/\#?[0-9ABCDEFabcdef]{6}/", $elem->background))
                    $elem->background = '';
                $elem->required   = true;
                ...
                if (@$elem->extensions != '') {
                    ...
                    if (is_array($tmpExts) && count($tmpExts) > 0) {
                        foreach ($tmpExts as $tmpExt) {
                            $tmpExt = trim($tmpExt);
                            if (preg_match('/^[a-zA-Z0-9]{2,4}$/', $tmpExt))
                                $exts[] = $tmpExt;
                        }
                    }
                }
    ...
    function renderElement($elem) {
        ...
        $label       = '';
        if ($elem->label != '')
            $label       = '<label for="' . $elem->id . '">' . $elem->label . ($elem->required ? ' <span>*</span>' : '') . '</label> ';
        switch ($elem->type) {
            case 'text':
            ...
            case 'select':
                ...
                foreach ($elem->options as $option) {
                    $sel        = '';
                    if ($option->selected || (count($elem->requests) && in_array($option->value, $elem->requests)))
            ...
            case 'captcha':
                ...
                if ((int) $user->get('id') > 0) {
                    ...
                    $result.= '<img width="197" class="sf2Captcha" src="' . JURI::root() . 'modules/mod_simpleform2/index.php?task=captcha' . (count($urlAdd) ? '&' . implode('&', $urlAdd) : '') . '" alt="' . JText::_('Click to refresh') . '" title="' . JText::_('Click to refresh') . '" onclick="' . $onclick . '"' . ' style="cursor:pointer;" />
    ...
    function checkDomain() {
        if (!function_exists('bcpowmod'))
            return true;
        $URI  = JURI::getInstance();
        $keys = explode('|', $this->_key);
        foreach ($keys as $key) {
            $m       = str_replace('www.', '', $URI->getHost()) . ':ZyX_SF2';
            $e       = 5;
            $n       = '159378341817953177';
            $s       = 5;
            $coded   = '';
            $max     = strlen($m);
            $packets = ceil($max / $s);
    ...
    function check(&$form, $request) {
        ...
        if (in_array($this->type, array('text', 'textarea'))) {
            ...
            if ($this->required && !is_file($fileData['tmp_name'])) {
                $this->setError($this->error);
                return false;
            } else if (!is_file($fileData['tmp_name']))
                return true;
