						...
						switch ($N) 
							{
								...
								else if ($this->Gender() == GENNEUTER) { return 'один'; }
		...
		function CountOfUnits($AUnit = array('Gender'=>GENNEUTER,'Base'=>'','End1'=>'','End2'=>'','End5'=>'',), $N,$Options = array()) 
			{
				...
				if (@in_array('ntoDigits',$Options)) 
				...
				else
					{
						...
						if ($Un > 0) { $result .= $this->ConvertToText($AUnit,$Un); }
		...
		function CurrencyToText($sum,$price = "RUR") 
			{
				...
				switch ($price) {
				case 'USD' : $sprice = $this->WD_USD; $sprice1 = $this->WD_CENT; break;
				case 'KGS' : $sprice = $this->WD_SOM; $sprice1 = $this->WD_TIYIN; break;
				}
