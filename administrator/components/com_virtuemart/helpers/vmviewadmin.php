...
//]]>
    ...
    function displayDefaultViewSearch($searchLabel = 'COM_VIRTUEMART_NAME', $name = 'search', $addSN = false)
    {
        $html = vmText::_($searchLabel).':';
        $html .= '<input type="text" name="'.$name.'" id="'.$name.'" value="'.$this->lists[$name].'" class="text_area" /> ';

        if ($addSN) {
            $html .= '<input type="checkbox" name="is_serial" id="is_serial" /> '.vmText::_('COM_VIRTUEMART_SERIAL_NUMBER').' ';
        }

        $html .= '<button class="btn btn-small" onclick="this.form.submit();">'.vmText::_('COM_VIRTUEMART_GO').'</button> ';
        $html .= '<button class="btn btn-small" onclick="document.getElementById(\''.$name.'\').value=\'\';this.form.submit();">'.vmText::_('COM_VIRTUEMART_RESET').'</button>';
    ...
    {
        if (!$name)
            $name = 'COM_VIRTUEMART_'.strtoupper($orderby);
