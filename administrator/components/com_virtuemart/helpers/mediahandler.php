...
class VmMediaHandler {
    ...
    public function displayFoldersWriteAble() {
        ...
        foreach ($this->_foldersToTest as $dir) {
            $result .= $dir . ' :: ';
            $result .= is_writable($dir) ? '<span style="font-weight:bold;color:green;">' . vmText::_('COM_VIRTUEMART_WRITABLE') . '</span>' : '<span style="font-weight:bold;color:red;">' . vmText::_('COM_VIRTUEMART_UNWRITABLE') . '</span>';
            $result .= '<br/>';
        }
        $result .= '</div>';
        return $result;
    ...
    function displayMediaThumb($imageArgs = '', $lightbox = true, $effect = "class='modal' rel='group'", $return = true, $withDescr = false, $absUrl = false, $width = 0, $height = 0) {
        ...
        if (strpos($file_url_thumb, 'http') !== false) {
            return $this->displayIt($file_url_thumb, $file_alt, $imageArgs, $lightbox, $effect, $withDescr, $absUrl);
        } else if ((empty($file_url_thumb) || !file_exists($media_path)) && is_a($this, 'VmImage')) {
        ...
        if ($withDescr) {
            $withDescr = $this->file_description;
        }
    ...
    function displayIt($file_url, $file_alt, $imageArgs, $lightbox, $effect = "class='modal'", $withDesc = false, $absUrl = false) {
        ...
        if ($withDesc)
            $desc = '<span class="vm-img-desc">' . $withDesc . '</span>';
        else
            $desc = '';
