...
class VirtueMartModelOrders extends VmModel
{
    ...
    public function getOrderIdByOrderPass($orderNumber, $orderPass)
    {
        ...
        if (empty($orderId))
            vmdebug('getOrderIdByOrderPass no Order found $orderNumber = '.$orderNumber.' $orderPass = '.$orderPass);
        return $orderId;
    }
...
// 		vmdebug('getOrder my order',$order);
    ...
    {
        $isSerial = vRequest::getString('is_serial', false);
        ...
        if ($search && $isSerial) {
            ...
            $where[] = ' o.virtuemart_order_id IN ('
                    .'SELECT oi.virtuemart_order_id '
                    .'FROM #__virtuemart_order_items oi '
                    .'WHERE oi.product_attribute LIKE '.$search.') ';
        } elseif ($search) {
            $db     = JFactory::getDBO();
            $search = '"%'.$db->escape($search, true).'%"';
    ...
    {
        $from = ' FROM #__virtuemart_orders as o
            ...
            if (!empty($calc_rules)) {
                foreach ($calc_rules as $calc_kind => $calc_rule) {
                    foreach ($calc_rule as $virtuemart_order_calc_rule_id => $calc_amount) {
                        ...
                        $db->setQuery($sql);
                        if (isset($calc_amount))
                            $calc_rules_amount += $calc_amount;
...
// 			}
    ...
    private function _createOrder($_cart, $_usr)
    {
        ...
        $db->setQuery('SELECT * FROM `#__virtuemart_currencies` '
                .'WHERE `currency_code_3`= "KGS" ');
        $currencyKgs = $db->loadObject();

        $_orderData->nrt          = 0;
        $_orderData->exchange_usd = isset($currencyKgs->currency_exchange_rate) ? $currencyKgs->currency_exchange_rate : 0;
        $_orderData->delivery     = isset($currencyKgs->currency_delivery) ? $currencyKgs->currency_delivery : 0;
    ...
    private function _createOrderCalcRules($order_id, $_cart)
    {
        ...
        foreach ($productKeys as $key) {
            foreach ($calculation_kinds as $calculation_kind) {
                ...
                if (!isset($_cart->cartPrices[$key][$calculation_kind]))
                    continue;
