    ...
    function createMediaByIds($virtuemart_media_ids, $type = '', $mime = '', $limit = 0) {
        ...
        if (!empty($virtuemart_media_ids)) {
            if (!is_array($virtuemart_media_ids))
                $virtuemart_media_ids = explode(',', $virtuemart_media_ids);
            ...
            foreach ($virtuemart_media_ids as $k => $virtuemart_media_id) {
                if ($limit !== 0 and $k == $limit and !empty($medias))
                    break; // never break if $limit = 0
                    ...
                    if (!array_key_exists($id, $_medias)) {
                        ...
                        if ($app->isSite()) {
                            ...
                            if (in_array($lang->getTag(), $selectedLangue) || $data->file_lang == '') {
                                ...
                                if (is_object($virtuemart_media_id) && !empty($virtuemart_media_id->product_name))
                                    $_medias[$id]->product_name = $virtuemart_media_id->product_name;
                            }
                        } else {
                            $_medias[$id]               = VmMediaHandler::createMedia($data, $file_type, $mime);
                            if (is_object($virtuemart_media_id) && !empty($virtuemart_media_id->product_name))
    ...
    public function attachImages($objects, $type, $mime = '', $limit = 0) {
        if (!empty($objects)) {
            ...
            foreach ($objects as $k => $object) {
                ...
                if (empty($object->virtuemart_media_id))
                    $virtuemart_media_id = null;
                else
                    $virtuemart_media_id = $object->virtuemart_media_id;
