...
class shopFunctionsF
{
    ...
    static public function getLoginForm($cart = FALSE, $order = FALSE, $url = '')
    {
        ...
        if ($show == 1) {
            ...
            if (!class_exists('VirtuemartViewUser'))
                require(VMPATH_SITE.DS.'views'.DS.'user'.DS.'view.html.php');
    ...
    static public function getAddToCartButton($orderable)
    {
        ...
        if ($orderable) {
            $html = '<input type="submit" name="addtocart" class="addtocart-button button-gloss" value="'.vmText::_('COM_VIRTUEMART_CART_ADD_TO').'" title="'.vmText::_('COM_VIRTUEMART_CART_ADD_TO').'" />';
        } else {
...
// 		$format = (VmConfig::get('order_html_email',1)) ? 'html' : 'raw';
    ...
    private static function sendVmMail(&$view, $recipient, $noVendorMail = FALSE)
    {
        ...
        $mailer->setSender($sender[1],$sender[1]);
