    ...
    foreach ($this->assembly as $assembly) {
        ...
        <?php if (!empty($assembly->description)) {
            ...
            <div class="category_description">
                <?php echo html_entity_decode($assembly->description); ?>
            </div>
            <?php
        }
        ?>
        <div class="browse-view">
            ...
            if (!empty($assembly->products)) {
                ...
                echo shopFunctionsF::renderVmSubLayout('products_assembly', array('products' => $products, 'currency' => $assembly->currency, 'products_per_row' => $this->perRow, 'showRating' => 0));
            ...
            foreach ($assembly->products as $product) {
                $allPrices['kgs'] += round($assembly->currency->roundForDisplay($product->prices['salesPrice'], 165, 1, 0)) * $assembly->assemblyProducts[$product->virtuemart_product_id]->product_count;
                $allPrices['usd'] += round($product->prices['salesPrice'], 2) * $assembly->assemblyProducts[$product->virtuemart_product_id]->product_count;
            }
            ?>
            ...
            <div class="addtocart-area" style="width:100%; height: 50px;float: right;">
                <form action="/newenter/index.php?option=com_virtuemart" class="product js-recalculate" method="post">
                    ...
                    <?php
                    $i = 0;
                    foreach ($assembly->assemblyProducts as $item) {
                        ?>
